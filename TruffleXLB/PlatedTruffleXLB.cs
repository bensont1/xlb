namespace KitchenXLB.Mains
{
    internal class PlatedTruffleXLB : CustomItemGroup<TruffleXLB.View>
    {
        public override string UniqueNameID => "plated_truffle_xlb";
        public override GameObject Prefab => Mod.Bundle.LoadAsset<GameObject>("truffle_xlb_plated");
        public override ItemStorage ItemStorageFlags => ItemStorage.None;
        public override ItemCategory ItemCategory => ItemCategory.Generic;
        public override ItemValue ItemValue => ItemValue.Large;

        public override bool CanContainSide => true;
        public override Item DisposesTo => GetGDO<Item>(ItemReferences.Plate);
        public override Item DirtiesTo => GetGDO<Item>(ItemReferences.PlateDirty);

        public override List<ItemGroupView.ColourBlindLabel> Labels => new()
        {
            new()
            {
                Item = GetCastedGDO<Item, PlainTruffleXLB>(),
                Text = "TrXlb"
            },
        };

        public override List<ItemGroup.ItemSet> Sets => new()
        {
            new()
            {
                Max = 2,
                Min = 2,
                IsMandatory = true,
                Items = new()
                {
                    GetCastedGDO<Item, TruffleXLB>(),
                    GetGDO<Item>(ItemReferences.Plate)
                }
            }
        };

        public override void OnRegister(ItemGroup gdo)
        {
            Prefab.ApplyMaterialToChildGame("basket", "XLB - \"Basket\"");
            Prefab.ApplyMaterialToChildGame("baos", "Oyster");
            Prefab.ApplyMaterialToChildGame("plate", "Plate", "Plate - Ring");
        }
    }
}